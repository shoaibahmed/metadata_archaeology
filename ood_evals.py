...
print(f"Performing OOD detection using trajectory...")
loss_fn_dict = {"max-logit": MaxLogitLoss(), "sorted-logits": SortedLogitLoss()}
loss_fn_list = natsort.natsorted(list(loss_fn_dict.keys()))
print("Loss fn list:", loss_fn_list)
...
tensor_shape = probes_ood_det["id"].shape
probes_ood_det["random_ood"] = torch.clip(torch.randn(*tensor_shape), 0., 1.).to(device)
...
if not use_model_pred:
    ...
    probes_ood_det["random_ood_labels"] = torch.randint(0, num_classes, (num_example_probes,)).to(device)
...
if use_model_pred:
    ...
    with torch.no_grad():
        ...
        logits = model(probes_ood_det["random_ood"])
        probes_ood_det["random_ood_labels"] = logits.argmax(dim=1)
...
id_trajectories_dict = {}
ood_trajectories_dict = {}
random_ood_trajectories_dict = {}
clf_dict = {}
random_clf_dict = {}
oc_clf_dict = {}
...
for loss_fn in loss_fn_list:
    ...
    for epoch in range(num_epochs):
        ...
        id_stats, id_preds = test_tensor(model_replicas[epoch], device, loss_fn_dict[loss_fn], probes_ood_det["id"], probes_ood_det["id_labels"],
                                         msg="In-distribution detection probe", log_predictions=True)
        ood_stats, ood_preds = test_tensor(model_replicas[epoch], device, loss_fn_dict[loss_fn], probes_ood_det["ood"], probes_ood_det["ood_labels"],
                                           msg="Out-of-distribution detection probe", log_predictions=True)
        random_ood_stats, random_ood_preds = test_tensor(model_replicas[epoch], device, loss_fn_dict[loss_fn], probes_ood_det["random_ood"], probes_ood_det["random_ood_labels"],
                                                         msg="Random out-of-distribution detection probe", log_predictions=True)
        id_trajectories.append(id_preds["loss_vals"])
        ood_trajectories.append(ood_preds["loss_vals"])
        random_ood_trajectories.append(random_ood_preds["loss_vals"])

    # Convert the loss values into a trajectory
    if loss_fn == "sorted-logits":
        ...
        random_ood_trajectories = np.array(random_ood_trajectories).transpose(0, 2, 1).reshape(-1, num_ood_ex)
        print(f"!! Reshaped sorted logits / ID trajectories: {np.array(id_trajectories).shape} / OOD trajectories: {np.array(ood_trajectories).shape} / Random OOD trajectories: {random_ood_trajectories.shape}")

    id_trajectories = np.transpose(np.array(id_trajectories), (1, 0))
    ood_trajectories = np.transpose(np.array(ood_trajectories), (1, 0))
    random_ood_trajectories = np.transpose(np.array(random_ood_trajectories), (1, 0))
    print(f"ID trajectories: {id_trajectories.shape} / OOD trajectories: {ood_trajectories.shape} / Random OOD trajectories: {random_ood_trajectories.shape}")

    # Plot the trajectories
    trajectories_dict = dict(id=id_trajectories, ood=ood_trajectories, random_ood=random_ood_trajectories)
    label_map_dict = dict(id="ID", ood="OOD", random_ood="OOD (Random)")
    output_file = os.path.join(experiment_output_dir, f"loss_trajectories_imagenet_id_vs_ood_{ood_dataset_name}_{loss_fn}.png")
    y_label = "Loss values"
    ...
    if loss_fn == "max-logit":
        y_label = "Max logit"
    elif loss_fn == "sorted-logits":
    ...
    # Train the random k-NN classifier
    ...
    # Append the details into the dict
    id_trajectories_dict[loss_fn] = id_trajectories
    ood_trajectories_dict[loss_fn] = ood_trajectories
    clf_dict[loss_fn] = clf
    random_clf_dict[loss_fn] = random_clf
    oc_clf_dict[loss_fn] = oc_clf
...
for loader_idx, loader in enumerate([test_set_sel_loader, ood_dataset_sel_loader]):
    ...
    targets = []
    max_logits = []
    predictions = {k: [] for k in loss_fn_list}
    random_predictions = {k: [] for k in loss_fn_list}
    avg_dists = {k: [] for k in loss_fn_list}
    id_pred = {k: 0 for k in loss_fn_list}
    ood_pred = {k: 0 for k in loss_fn_list}
    random_id_pred = {k: 0 for k in loss_fn_list}
    random_ood_pred = {k: 0 for k in loss_fn_list}
    total = {k: 0 for k in loss_fn_list}
    ...
    for x, y in tqdm(loader):
        ...
        for loss_fn in loss_fn_list:
            ...
            for epoch in range(num_epochs):
                # Compute the loss trajectory on all probe examples
                current_stats, current_preds = test_tensor(model_replicas[epoch], device, loss_fn_dict[loss_fn], x, y,
            ...
            traj_preds = clf_dict[loss_fn].predict_proba(current_trajectories)
            assert len(traj_preds.shape) == 2, traj_preds.shape
            assert traj_preds.shape == (len(current_trajectories), 2), traj_preds.shape
            
            # Use the random input classifier
            random_traj_preds = random_clf_dict[loss_fn].predict_proba(current_trajectories)
            assert len(random_traj_preds.shape) == 2, random_traj_preds.shape
            assert random_traj_preds.shape == (len(current_trajectories), 2), random_traj_preds.shape
            
            # Perform inference on the ID trajectories
            neighbors_dist, _ = oc_clf_dict[loss_fn].kneighbors(current_trajectories)
            ...
            avg_dists[loss_fn].append(avg_dist)
            
            # Evaluate the prediction
            id_pred[loss_fn] += np.sum(traj_preds[:, 1] < thresh)
            ood_pred[loss_fn] += np.sum(traj_preds[:, 1] >= thresh)
            random_id_pred[loss_fn] += np.sum(random_traj_preds[:, 1] < thresh)
            random_ood_pred[loss_fn] += np.sum(random_traj_preds[:, 1] >= thresh)
            predictions[loss_fn].append(traj_preds[:, 1])
            random_predictions[loss_fn].append(random_traj_preds[:, 1])
            total[loss_fn] += len(current_trajectories)
        
        # Compute max-logit
        with torch.no_grad():
            logits = model_replicas[num_epochs-1](x)
    ...
    for loss_fn in loss_fn_list:
        ...
        assert random_id_pred[loss_fn] == np.sum(random_predictions[loss_fn] < thresh), random_id_pred[loss_fn]
        assert random_ood_pred[loss_fn] == np.sum(random_predictions[loss_fn] >= thresh), random_ood_pred[loss_fn]
        print(f">> Final stats / Dataset: {'ID' if loader_idx == 0 else 'OOD'} / Loss fn: {loss_fn} / Total: {total[loss_fn]} / ID preds: {id_pred[loss_fn]} / ", end='')
        print(f"OOD preds: {ood_pred[loss_fn]} / Random ID preds: {random_id_pred[loss_fn]} / Random OOD preds: {random_ood_pred[loss_fn]}")
